		...
		<div id="Inquire-two">
			...
			<form id="searchFrom">
				<div class="input-group">
					...
					<input id="name" name="name" usemap="{'logic':'and','compare':'like'}" placeholder="请输入企业名称" /> 
			...
			<div>
				<!-- 添加企业按钮 -->
				<p class="send">
					<button class="btn btn-zdy" data-toggle="modal" data-target="#addcontent" onclick="addcontent();">添加企业</button>
				</p>
					...
					<form id="add_formId" enctype="multipart/form-data" class="form-horizontal">
						<div class="modal-body">
							<div class="form-group">
								<label for="contentname" class="col-sm-3 control-label">所属行业：</label>
								<div class="col-sm-4">
									<select id="add_select_industry" name="add_select_industry"
				...
				<div class="modal-body">
					<form id="edit_formId" class="form-horizontal" method="post">
						...
						<div class="form-group">
							<label for="contentname" class="col-sm-3 control-label">所属行业：</label>
							<div class="col-sm-4">
								<select id="edit_select_industry" name="edit_select_industry"
									style="height: 33px; width: 100%; border-color: #e3e6f3;">
									<option value="0">请选择</option>
								</select>
							</div>
			...
			new CuiAjaxForm($('#add_formId'), {
				...
				beforeEvent : function(formData, jqForm, options) {
					...
					$.insertDynamicDataForTheForm(formData, jqForm,
							options, 'pFile', 'file');
					$.insertDynamicDataForTheForm(formData, jqForm,
							options, 'pKey', 'insertFileCompany');
					return true;
				},
			...
			new CuiAjaxForm($('#edit_formId'), {
				...
				beforeEvent : function(formData, jqForm, options) {
					...
					$.insertDynamicDataForTheForm(formData, jqForm,
							options, 'pFile', 'file');
					$.insertDynamicDataForTheForm(formData, jqForm,
							options, 'pKey', 'updateFileCompany');
					return true;
				},
				callbackEvent : function(result) {
					if (result.status == 'success') {
	...
	$.asyncRequest({
		...
		event : function(result) {
			if (result.status === 'success') {
				...
				//下面写你的任何业务
				//alert(JSON.stringify(pageSource));
				$("#add_select_industry").html("");// 清空
				$("#edit_select_industry").html("");// 清空
				var targetHtml = "";
				targetHtml += "<option value='0'>请选择</option>";
				for (var i = 0; i < pageSource.length; i++) {
					targetHtml += "<option value='"+pageSource[i].id+"'>" + pageSource[i].name + "</option>"
				}
				$("#add_select_industry").html(targetHtml);
				$("#edit_select_industry").html(targetHtml);
			}
		},
	});
}
	...
	$.ajax({
		...
		success : function(result) {
			if (result.status === 'success') {
				...
				$("#edit_select_industry").val(listInfo[0].ts_industry_id);
