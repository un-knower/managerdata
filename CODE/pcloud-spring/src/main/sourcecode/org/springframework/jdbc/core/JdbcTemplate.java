...
import org.pcloud.spring.jdbc.utils.MapRowMapper;
...
public class JdbcTemplate extends JdbcAccessor implements JdbcOperations {
	...
	private String getBuildedSql(Connection connection, final String sql, final Object[] params) throws Exception {
		LoggableStatement psst =  new LoggableStatement(connection,sql); 
		if (null != params) {
			for (int i = 0; i < params.length; i++) {
				psst.setObject(i + 1, params[i]);
			}
		}
		String custSql = psst.getSqlString();
		psst.close();
		System.out.println(custSql);
		return custSql;
	}
	
	public int[] batchUpdate(final String[] sql,final Object[] params) throws DataAccessException {
			...
			public int[] doInStatement(Statement stmt) throws SQLException, DataAccessException {
				...
				if (JdbcUtils.supportsBatchUpdates(con)) {
					for (int i = 0; i < sql.length; i++) {
						String sqlStmt;
						try {
							sqlStmt = getBuildedSql(con, sql[i], (Object[])params[i]);
						} catch (Exception e) {
							sqlStmt = sql[i];
						}
					...
					for (int i = 0; i < sql.length; i++) {
						try {
							this.currSql = getBuildedSql(con, sql[i], (Object[])params[i]);
						} catch (Exception e) {
							this.currSql = sql[i];
						}
		...
		return execute(psc, new PreparedStatementCallback<T>() {
			...
			public T doInPreparedStatement(PreparedStatement ps) throws SQLException {
				...
				try {
					...
					System.out.println(ps.toString().substring(ps.toString().indexOf(":")+1));
					rs = ps.executeQuery();
